...
const Cart = () => {
  ...
  const fetchAvailableCoupons = async () => {
    ...
    try {
      const res = await axios.get("http://localhost:3000/api/coupons/user/available", {
          ...
          {cartItems.map((item) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <h2 className="font-semibold text-gray-800 text-lg truncate">{item.name}</h2>
                <p className="text-indigo-600 font-bold text-lg mt-1">₹{item.price}</p>
                ...
                <div className="flex items-center gap-3 mt-3">
                  <button
                    onClick={() => decreaseQty(item.productId, item.size, item.color)}
                  ...
                  <button
                    onClick={() => increaseQty(item.productId, item.size, item.color)}
                    className="w-8 h-8 flex items-center justify-center bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition-colors cursor-pointer"
                  >
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div className="text-left">
                  <h3 className="font-semibold text-gray-800 text-sm">Available Coupons</h3>
              ...
              <div className="border-t border-gray-100 p-4 space-y-3">
                {couponsLoading ? (
                  ...
                  <div className="text-center py-6">
                    ...
                    <p className="text-gray-400 text-sm">No coupons available right now</p>
                  </div>
                ) : (
                  availableCoupons.map((coupon) => {
                    ...
                    const meetsMinOrder = totalAmount >= coupon.minimumOrderAmount;
                    const isClickable = coupon.isEligible && meetsMinOrder && !isApplied;

                    return (
                      <div key={coupon._id}>
                        <div
                          onClick={() => !isApplied && !isApplying && handleCouponCardClick(coupon)}
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                {isApplying && <Loader2 className="w-4 h-4 animate-spin text-indigo-500" />}
                              </div>

                              {coupon.description && (
                                <p className="text-sm text-gray-600 mb-1.5 line-clamp-1">{coupon.description}</p>
                              )}
                                ...
                                {coupon.minimumOrderAmount > 0 && (
                                  <span
                                    className={`text-[11px] flex items-center gap-1 ${
                                      meetsMinOrder ? "text-gray-400" : "text-red-400 font-medium"
                                    }`}
                                  >
                        ...
                        {hasError && (
                          <div className="flex items-start gap-1.5 mt-1.5 ml-1 text-red-500 animate-in slide-in-from-top-1">
                            ...
                            <p className="text-xs leading-relaxed">{hasError}</p>
                          </div>
                        )}
                        ...
                        {!coupon.isEligible && !hasError && (
                          <div className="flex items-start gap-1.5 mt-1.5 ml-1 text-gray-400">
                            <AlertCircle className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                            <p className="text-xs leading-relaxed">{coupon.reason}</p>
                          </div>
                        )}
                      </div>
                    );
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-6">
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="text-right">
                {appliedCoupon && (
                  <p className="text-sm text-gray-400 line-through mb-0.5">₹{totalAmount}</p>
                )}
                <p className="text-2xl font-bold text-gray-800">₹{finalAmount}</p>
              </div>
            </div>
            ...
            {appliedCoupon && (
              <div className="bg-linear-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl px-4 py-2.5 mb-4 text-center">
                <p className="text-emerald-700 text-sm font-medium">
                   You're saving <span className="font-bold">₹{appliedCoupon.discount}</span> on this order!
                </p>
              </div>
            )}
